...
- `--field NAME` (repeatable) projects only the listed fields out of the response; `--device mobile|desktop` flattens the named device-metrics block alongside the identifier fields. The two compose: `--device` narrows scope, `--field` projects within it, and top-level identifier fields (`publisher_id`, `name`, `domain`) are always preserved when `--device` is set.
- `--api-key TOKEN` and `--timeout SECONDS` flags on `opensincera get` for ad-hoc overrides without mutating the environment.

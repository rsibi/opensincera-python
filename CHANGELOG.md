...
- `opensincera` CLI (Typer-based): `opensincera get <domain>` and `opensincera get --id <id>` print the publisher as indented JSON. Reads the API key from the `OPENSINCERA_API_KEY` environment variable. `--version` and shell-completion install via `--install-completion` ship for free.
